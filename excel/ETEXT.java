package writeExcel;
//  = ETEXT.class  just check samples
...
public class ETEXT {
	...
	public static void main(String[] args) throws Exception {
		...
		File src = new File ("C:\\Users\\vkoloyu\\workspace\\proj\\src\\writeExcel\\XLS\\c.xlsx");
		...
		for (int i=1; i<=rowcount; i++)
		{
			...
			Thread.sleep(3000);
				...
				if (driver.findElements(By.xpath("//*[@id='_loginName']")).size() != 0) {
					...
					sheet1.getRow(i).createCell(5).setCellValue("sign in");
					FileOutputStream fout = new FileOutputStream (src);
					wb.write(fout);	
					System.out.println(" sign in");
					
				} else {
					sheet1.getRow(i).createCell(5).setCellValue("passed");
					FileOutputStream fout = new FileOutputStream (src);
					wb.write(fout);	
